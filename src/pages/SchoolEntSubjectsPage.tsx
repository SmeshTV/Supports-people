...
import { supabase, type Direction, type Section } from '../lib/supabase';
...
export default function SchoolEntSubjectsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const [{ data: dir }, { data: secs }] = await Promise.all([
        supabase.from('directions').select('*').eq('id', subjectId).maybeSingle(),
        supabase.from('sections').select('*').eq('direction_id', subjectId).eq('is_published', true).order('order_index')
      ]);
